...
if ($pdfs.Count -eq 0) { Log 'no PDFs downloaded (NH likely published nothing new). Done.'; & ssh docketdrift 'date -u +%s > /home/private/docketdrift/.scrape_nh_last' 2>&1 | Out-Null; exit 0 }
...
& ssh docketdrift 'date -u +%s > /home/private/docketdrift/.scrape_nh_last' 2>&1 | Out-Null
