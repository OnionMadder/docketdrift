...
@admin.action(description="Holdings: mark holding human-reviewed")
def mark_holding_reviewed(modeladmin, request, queryset):
    """Bulk-flip the extracted HOLDING to REVIEWED (cyan dot), stamping editor.
    ...
    modeladmin.message_user(request, f"{n} holding(s) marked human-reviewed.")


@admin.action(description="Holdings: flag holding for review")
def flag_holding(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{n} holding(s) flagged.")
...
def requeue_holding_extraction(modeladmin, request, queryset):
    """Clear the summary so the next default extract_holdings_text run regenerates it.
...
@admin.action(description="Holdings: revert holding to Processed (un-review)")
def revert_holding_ai_only(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{n} holding(s) reverted to Processed.")
...
class OpinionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Holding (extracted verbatim)", {
            ...
            "description": (
                "The court's own holding sentence, extracted verbatim "
                "(extract_holdings_text) -- not a machine-written summary. "
                "holding_summary is editable -- fix a mis-extracted sentence or "
                "a stray artifact here and set holding_review_status to "
                "Human-reviewed to publish it as curated (auto-stamps "
                "holding_reviewed_at on save). For high-throughput passes use "
                "the holdings-review surface at "
