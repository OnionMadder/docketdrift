...
# ----------------------------------------------------------------------
# Holdings review surface -- the same high-throughput HTMX flow as tag
# review, applied to the verbatim-extracted holdings (extract_holdings_text,
# the court's OWN words -- not a machine-written summary). Onion sits down
# with the AI_ONLY queue, reads each holding against the opinion, fixes a
# mis-extracted sentence or stray artifact in place if needed, and flips it
# to human-REVIEWED -- 50/hour, no full-page reloads. All three states carry
# extracted holdings now.
# ----------------------------------------------------------------------
...
def holding_review(request):
    """Render the bulk holding review grid.
...
def holding_review_action(request, opinion_id: int, action: str):
    ...
    Actions (POSTed via HTMX from the per-row buttons):
      ...
      requeue  -> clear the summary + stamps so the next DEFAULT extract_holdings_text
    ...
    if action == "review":
        ...
        # Clear the summary so the next default extract_holdings_text run picks it
